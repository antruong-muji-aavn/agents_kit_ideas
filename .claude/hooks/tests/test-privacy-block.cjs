...
const CK_CONFIG_PATH = path.join(__dirname, '..', '.epost-kit.json');
...
async function main() {
  ...
  // Config toggle tests - requires temp directory with .claude/.epost-kit.json
  ...
  for (const test of configToggleTests) {
    // Write test config
    fs.writeFileSync(
      path.join(tmpClaudeDir, '.epost-kit.json'),
