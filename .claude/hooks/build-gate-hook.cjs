 ...
 *   - `EPOST_SKIP_BUILD=1` environment variable
 *   - `hooks['build-gate'].enabled = false` in .epost-kit.json
 *   - `EPOST_BUILD_GATE_RAN=1` env (dedup — skill already ran gate)
 */
...
try {
  ...
  if (process.env.EPOST_BUILD_GATE_RAN === '1') {
  ...
  // EPOST_SKIP_BUILD env var
  if (process.env.EPOST_SKIP_BUILD === '1') {
    process.stderr.write('build-gate-hook: skipping (EPOST_SKIP_BUILD=1)\n');
    process.exit(0);
  }
  ...
  process.stderr.write(
    ...
    `To bypass (WIP/draft): add --skip-build or set EPOST_SKIP_BUILD=1\n`
  );
  process.exit(2);
